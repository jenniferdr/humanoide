...
\end{figure}
Las posibles combinaciones que se pueden generar con las acciones es $2 ^ 9$ por lo cual se considero s\'olo un subconjunto de estas combinaciones aquellas que el robot realiza con mas frecuencia al caminar.

Para esta segunda etapa se evalu\'o la ejecuci\'on de combinaciones de una serie de unidades de movimiento. Las combinaciones elegidas fueron: 
...
%\label{fig:etp2}
%\end{figure}
Para determinar la presi\'on y desempe\~no de la detecci\'ion de la pelota se realizaron experimentos de enfoque r\'apido los cuales consistieron en la colocaci\'on de la pelota a una distancia fija, mover la c\'amara rapidamente y regresar a la posici\'on y observar si ubicaba la pelota correctamente, las distancias utilizadas fueron 50cm y 80cm del robot. Adem\'as para determinar posibles Falsos Positivos, se refiere a detectar una pelota inexistente, se realiz\'o el mismo experimento pero sin la pelota. Los resultados obtenidos se prsentan en la siguiente tabla ~\ref{fig:deteccion}.
...
\end{figure}
 Un resultado a destacar es que el tiempo promedio de pruebas a la derecha (Caso IV) es en proporci\'on mucho mayor a las pruebas a la izquierda (Caso II) a pesar que las mismas son sim\'etricas esto nos indica que los siguientes entrenamientos deben hacer \'enfasis en los estados a la derecha del robot. 
